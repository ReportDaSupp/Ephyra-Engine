...
namespace Ephyra
{
	...
	void Ephyra::RandomNumGen::start(SystemSignal init, ...)
	{
		auto now = std::chrono::high_resolution_clock::now();
		s_generator.reset(new std::mt19937(now.time_since_epoch().count()));
	}

	void Ephyra::RandomNumGen::stop(SystemSignal close, ...)
	{
		s_generator.reset();
	}

	int32_t Ephyra::RandomNumGen::uniformIntBetween(int32_t lower, int32_t upper)
	{
		return lower + 2*(fabs(static_cast<float>(s_uniformInt(*s_generator))) / (s_intRange))*(upper - lower);
	}

	float Ephyra::RandomNumGen::uniformFloatBetween(float lower, float upper)
	{
