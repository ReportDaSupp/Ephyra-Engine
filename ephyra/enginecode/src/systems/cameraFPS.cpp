...
namespace Engine
{
	...
	FirstPersonCamera::FirstPersonCamera(glm::vec3 pos) : Camera(pos), m_speed(1.5f), m_sensitivity(0.075f), m_running(true)
	{
		m_zoom = 45.0;
		m_yaw = -90.0f;
		m_pitch = 0.0f;
	...
	//updates
	void FirstPersonCamera::attachWindow(std::shared_ptr<Window>& window, std::shared_ptr<InputPoller>& handler)
	{
		m_window = window;
		m_handler = handler;
	}

	glm::vec3 FirstPersonCamera::cameraShake()
	{
		float offset = perlin(shakeIncrementer);
